...
<template>
  <section>
    <!-- <pre class="text-xs font-bold">{{ dishVars }}</pre> -->
    ...
    <div
      v-for="type in types"
      :key="type.id"
      class="px-1 mt-2 border-b border-gray-300"
    >
      <span
        class="block pt-1 pb-2 mx-2 text-sm font-bold capitalize rounded cursor-pointer hover:text-gray-600"
        @click="settingsToshow = type.name"
      >
        <font-awesome-icon
          :icon="[
            'fas',
            settingsToshow === type.name ? 'chevron-up' : 'chevron-down',
          ]"
          class="mr-2"
        />
        {{ type.name }}
      ...
      <span
        v-if="!type.incremental"
        class="mx-2 text-xs text-orange-500 d-block"
      >
        This price will be replaced with the product price
      </span>

      <span
        v-show="settingsToshow === type.name"
      ...
      >
        ...
        <span class="flex w-full pb-2 m-1">
          <label
            :for="'yes_' + option.id"
          ...
          <!-- Set default Price -->
          <div
            class="flex items-center mx-1 text-xs font-bold text-gray-600"
            v-if="mainPrice == newoptionPrice[option.id] && !type.incremental"
          >
            <label class="cursor-pointer"> Default </label>
          </div>

          <!-- Select Side Dish type-->
          <select
            class="px-2 mx-2 text-xs font-bold text-blue-600 bg-white border border-blue-500 rounded cursor-pointer optionselected font- w-42 hover:bg-blue-100"
            name="sidedish"
          ...
          <!-- dish price -->
          <currency-input
            v-model="newoptionPrice[option.id]"
          ...
          <div
            ...
            v-if="
              !option.incremental &&
              is_sale > 0 &&
              mainPrice != newoptionPrice[option.id]
            "
          >
            <currency-input
...
export default {
  ...
  mounted() {
    this.types = this.$store.getters["products/getterTypes"];

    this.dishVars.forEach((element) => {
      element.options.forEach((type) => {
        //set id's to selected values to fill checkboxes
        ...
        if (!type.price_switch && !type.default_selection) {
          this.defaultSideDish[type.option_id] = undefined;
        } else if (!type.price_switch && type.default_selection) {
          this.defaultSideDish[type.option_id] = "defaultselect";
        } else if (type.price_switch && !type.default_selection) {
          this.defaultSideDish[type.option_id] = "priceswitch";
        }
        ...
        this.productVariations.push({
          ...
          default_selection: type.default_selection,
          side_dish: type.side_dish,
          price_switch: type.price_switch ? true : false,
        });
      });
    });
  },
  ...
  methods: {
    ...
    add(
      ...
      let object = {
        ...
        product_id: this.dishId,
        type_id: type,
        price: typeof newPrice !== "undefined" ? newPrice : oldPrice,
        sale_price: salePrice,
        default_selection: defaultSelection ?? false,
        price_switch: false,
        incremental,
        multi_select,
      };
    ...
    addVariations() {
      ...
      this.$axios
        .post(
          `menus/${menuSlug}/categories/${dish.category_slug}/products/${dish.slug}/variations`,
          {
        ...
        .then((res) => {
          setTimeout(() => {
            this.$store.commit("products/editDishVariations", res.data.data);
          }, 400);

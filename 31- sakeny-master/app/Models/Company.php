...
class Company extends Model
{
    ...
    public function setCrNumberFileAttribute($cr_number_file)
    {
        if (!$cr_number_file)
            return;
        $cr_number_file = $cr_number_file->store('uploads/comapnies', 's3');
    ...
    public function setLogoAttribute($logo)
    {
        if (!$logo)
            return;
        $logo = $logo->store('uploads/comapnies', 's3');
    ...
    {
        return $this->payments()->where('status', 1)->where('confirmed', 1)->where('expire_date', '>', Carbon::now())->first();
    }
    ...
    public function activesubscription()
    {
        ...
        return $this->hasOne(Subscription::class)->active()->approve()

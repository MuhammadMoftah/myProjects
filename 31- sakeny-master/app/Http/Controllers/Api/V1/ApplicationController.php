...
*/
    ...
    public function getSearchAds()
    {
        ...
        $ads = $ads->paginate(20);
        $template = null;
        if (request('country_id') && request('city_id') &&  request('offer_type_id') && request('property_type_id')) {
            $template = Template::where(['country_id' => request('country_id'), 'city_id' => request('city_id'), 'offer_type_id' => request('offer_type_id'), 'property_type_id' => request('property_type_id')])->first();
        }
        return $this->response(['data' => $ads, 'seo_template' => $template]);
    }

    public function templates()
    {
        $templates = Template::get();
        return $this->response(['data' => $templates]);
    }
    ...
    public function template($id)
    {
        ...
        $ads = Ads::active()->BusinessOrder()->Valid()
            ->where([
                ...
                'offer_type_id' => $template->offer_type_id,
    ...
    */
    ...
    {
        $rows = LifeStyle::latest()->select('id', 'title_ar', 'title_en', 'description_ar', 'description_en', 'image')->paginate(10);
        return $this->response(['data' => $rows]);
    }
    ...
    public function attachVideo($id)
    {
        ...
        $video = request()->file('video')->store('uploads','s3');

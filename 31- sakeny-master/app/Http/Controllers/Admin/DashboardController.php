...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $active_ads = Ads::where('status', 1)->whereDate('expire_date', '>=', Carbon::now())->count();
        $in_active_ads = Ads::where('status', 0)->whereDate('expire_date', '<', Carbon::now())->count();
        $expired_ads = Ads::whereDate('expire_date', '<', Carbon::now())->count();
        //        $stared_ads =
        //        $banners_num =
        //        $total_views =
        ...
        return view('admin.index', compact(
            'num_registred_users',
            'daily_registred_users',
            'num_registred_companies',
            'daily_registred_companies',
            'num_ads',
            'num_premium_ads',
            'active_ads',
            'in_active_ads',

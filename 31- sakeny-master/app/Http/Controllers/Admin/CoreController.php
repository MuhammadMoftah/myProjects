...
*/
	 ...
	 */
	protected function returnMessage($operation, $type, $custom_message = [] , $custom_route = "")
	{
		$route = $custom_route == "" ? $this->route : $custom_route;
		if($operation)
		{
			if($this->request->ajax())
				return response()->json(['status'=>'true','message'=>$this->successMessage($type, @$custom_message['success'])]);

			return redirect("admin/".$route)->with('success',$this->successMessage($type, @$custom_message['success']));
		}
		else
		{
			if($this->request->ajax())
			...
			return redirect("admin/".$route)->with('failed',$this->failedMessage($type, @$custom_message['failed']));
		}
	}
    ...
    {
        \Mail::send('emails.send', ['content' => $content], function ($message) use ($email, $subject)
        {
            $message->from('mail@sakeny.com', env('APP_NAME'));
	...
	protected function successMessage($type = '',$custom_message)
	{
		...
		switch ($type) {
			...
			case 4:
				return trans("lang.activatedsuccessfully");
				break;

			case 5:
				return trans("lang.de-activatedsuccessfully");
				break;

            ...
            <div class="">
                ...
                <ul class="nav navbar-nav navbar-right pull-right">
                    ...
                    @php
                        ...
                        $needed_company = $company_notifications->where("status",0);
                        $company_notification_loop = $company_notifications::query()->join('users', 'users.id','=','user_id')->where('users.activation',0)->get(['companies.*']);
                        $company_notification_count = $company_notification_loop->count();
                        ...
                        //echo '<br>sssssss<br>';die();
                        // company/view/approval-packages
                        //$company_approval_packages_count = $approval_packages->where('status',0)->count();
						{{--$company_approval_packages = $approval_packages->where('status',1)->where('confirmed',0)->get();
                        $company_approval_packages_count = $approval_packages->where('status',1)->where('confirmed',0)->count();--}}
                        //echo '<br>'.$company_approval_packages_count.'<br>';die();
						/*foreach($company_approval_packages as $company){
							echo '<pre>';
							var_dump($company->company_id);
							echo '</pre>';
						}
						die();*/
                    ...
                    {{--<li class="dropdown top-menu-item-xs">
                        ...
                        <ul class="dropdown-menu">
							<li style="box-shadow: 1px 3px 20px 1px;">
								<a href="#">
									<i class="ti-ruler-pencil text-custom m-r-10"></i> @lang('lang.Company Packages Approvals')
								</a>
							</li>
							@foreach ($company_approval_packages as $package)
							@php
							$company = $companyModel->where('user_id', $package->company_id)->first();
							@endphp
								<li>
									<a href="{{ url(ADMIN_PATH."/company_package/$company->id/list") }}">
										<i class="ti-announcement text-custom m-r-10"></i>{{ $company->registered_name }}
									</a>
								</li>
							@endforeach
                        </ul>
                   ...
                   <li class="dropdown top-menu-item-xs">
                        <a href="" style="color: #000 !important; font-weight: 700;" class="dropdown-toggle profile waves-effect waves-light" data-toggle="dropdown" aria-expanded="true"><img src="{{env('AWS_URL').auth()->guard('admin')->user()->image}}" alt="user-img" class="img-circle"> {{ auth()->guard('admin')->user()->name }}  <i class="fa fa-angle-down"></i></a>
                        <ul class="dropdown-menu">

...
class core extends mysqli {
	...
	var $db = "erasofte_man";
	var $username = "erasofte_man";
	var $password = "ndsdonnrsredfg{1]Z;";
	var $dbConnection;
	var $dbError;
	...
	function getprojectimage($pam) {
		...
		if (array_key_exists('project_id', $pam)) {
			$pam["project_id"] = (int)$pam["project_id"];
			$condition .= " and project_id = $pam[project_id]";
		}
		...
		while ($columns = $result -> fetch_array(MYSQLI_ASSOC)) {
			$typ["id"] = $columns["id"];
			$typ["image"] = $columns["image"];
			$typ["project_id"] = $columns["project_id"];
			$typ["order"] = $columns["order"];
			$typ["active"] = $columns["active"];
	...
	function getslider($sliderpram) {
		...
		while ($columns = $result -> fetch_array(MYSQLI_ASSOC)) {
			$slid["id"] = $columns["id"];
            $slid["name_arabic"] = $columns["name_arabic"];
			$slid["name_english"] = $columns["name_english"];
...
/*	function getfaq($pramfaq) {
	...
	function getservices($pramservices) {
		$query = "SELECT * FROM `services`";
		$condition = "active = 1 ";

		if (!empty($pramservices['id'])) {
			$pramservices['id'] = (int)$pramservices['id'];
			$condition .= "and  id = $pramservices[id]";
		}
	...
	//	print "$query $condition order by `order` asc";
		...
		while ($columns = $result -> fetch_array(MYSQLI_ASSOC)) {
			$serv["id"] = $columns["id"];
			$serv["name_english"] = $columns["name_english"];
			$serv["name_arabic"] = $columns["name_arabic"];
			$serv["order"] = $columns["order"];
			$serv["active"] = $columns["active"];
			$serv["image"] = $columns["image"];
			$serv["desc_arabic"] = $columns["desc_arabic"];
			$serv["desc_english"] = $columns["desc_english"];
	...
	function getnews($newspaaaarm) {
		...
		if (array_key_exists('special', $newspaaaarm)) {
			$newspaaaarm["special"] = (int)$newspaaaarm["special"];
			$condition .= "and  special = $newspaaaarm[special]";
		}
...
//***************************************************************************************
	...
	//***************************************************************************************
//***************************************************************************************
	function getproject_images($parm) {
		$query = "SELECT  * FROM `project_images`";
	...
	function getclients($parm) {
		...
		if (array_key_exists('client_category', $parm)) {
			$parm["client_category"] = (int)$parm["client_category"];
			$condition .= " and client_category = $parm[client_category]";
		}
		...
		while ($columns = $result -> fetch_array(MYSQLI_ASSOC)) {
			...
			$gall["name"] = $columns["name"];
			$gall["name_arabic"] = $columns["name_arabic"];
			$gall["client_category"] = $columns["client_category"];
			$gall["image"] = $columns["image"];
			$gall["description"] = $columns["description"];
			$gall["description_arabic"] = $columns["description_arabic"];
			$gall["order"] = $columns["order"];

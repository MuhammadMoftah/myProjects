  ...
  <div class="stocks-listing">
    <div v-if="$refs.table" class="buttons mb-3">
      <v-btn :disabled="loading || $refs.table.filteredItems.length === 0" color="primary">
        <DownloadExcel :data="$refs.table.filteredItems" :fields="generateExcelHeaders(headers)" title="Stocks" name="Stocks.xls" class="download-excel-btn">
          Export to Excel
        </DownloadExcel>
      </v-btn>
      <v-btn :disabled="loading || $refs.table.filteredItems.length === 0" color="primary" @click="generatePDF($refs.table.filteredItems, headers, 'Stocks')">
        Export to PDF
      </v-btn>
    </div>
    <v-toolbar flat class="elevation-1" color="white">
      ...
      <v-dialog v-model="dialog" :persistent="buttonLoading" content-class="dialog" max-width="500px">
        <v-btn v-if="$store.state.auth.user.permissions && $store.state.auth.user.permissions.includes('stock_create')" slot="activator" color="primary" dark class="mb-2">
          Add
        </v-btn>
    ...
    >
      <template slot="items" slot-scope="props">
        <nuxt-link tag="tr" :to="`/stock/${props.item.id}`">
          ...
          <td class=" d-none justify-center align-center layout px-0">
            <v-icon
              v-if="$store.state.auth.user.permissions && $store.state.auth.user.permissions.includes('stock_update')"
              small
              class="mr-2"
            ...
            <v-icon
              v-if="$store.state.auth.user.permissions && $store.state.auth.user.permissions.includes('stock_delete')"
              small
              class="mr-2"
...
export default {
  ...
  async mounted() {
    await this.$store.dispatch('stocks/getStocks')
    this.loading = false
  },
  methods: {
    close() {
    ...
    async del(item) {
      ...
      await this.$store.dispatch('stocks/deleteStock', item.id).catch(() => {
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        ...
        this.buttonLoading = true
        res = await this.$store
          .dispatch('stocks/newStock', this.editedItem)
          .catch(() => {
            this.buttonLoading = false
          })
        this.buttonLoading = false

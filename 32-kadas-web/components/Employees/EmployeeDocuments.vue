<template>
  <div class="employeeDocuments-listing">
    <div v-if="$refs.table" class="buttons mb-3">
      <v-btn :disabled="loading || $refs.table.filteredItems.length === 0" color="primary">
        <DownloadExcel :data="$refs.table.filteredItems" :fields="generateExcelHeaders(headers)" title="EmployeeDocuments" name="EmployeeDocuments.xls" class="download-excel-btn">
          Export to Excel
        </DownloadExcel>
      </v-btn>
      <v-btn :disabled="loading || $refs.table.filteredItems.length === 0" color="primary" @click="generatePDF($refs.table.filteredItems, headers, 'EmployeeDocuments')">
        Export to PDF
      </v-btn>
    </div>
...
export default {
  ...
  computed: {
    employeeDocuments() {
      return this.$store.state.employees.documents
    },
    formTitle() {
  ...
  async mounted() {
    ...
    await this.$store.dispatch('employees/getEmployeeDocuments')
    this.loading = false
  },
  methods: {
    handleUpload(e) {
    ...
    editItem(item) {
      this.editedIndex = this.employeeDocuments.indexOf(item)
    ...
    async del(item) {
      ...
      await this.$store
        .dispatch('employees/deleteEmployeeDocument', {
          id: this.id,
          docId: item.id
        })
        .catch(() => {
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        ...
        res = await this.$store
          .dispatch('employees/updateEmployeeDocument', {
            id: this.id,
            docId: this.employeeDocuments[this.editedIndex].id,
        ...
        this.buttonLoading = true
        res = await this.$store
          .dispatch('employees/newEmployeeDocument', {
            id: this.id,
            document: this.file,
            name: this.editedItem.name,
            type: this.editedItem.type

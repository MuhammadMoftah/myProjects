<template>
  <div>
    <div class="buttons mb-3">
      <v-btn :disabled="loading || loanTypes.length === 0" color="primary">
        <DownloadExcel
          :data="loanTypes"
      ...
      <v-btn
        ...
        color="primary"
        @click="generatePDF(loanTypes, headers, 'Loan Types')"
      >
        Export to PDF
      </v-btn>
    </div>
    <v-toolbar flat class="elevation-1" color="white">
      ...
      >
        <v-btn
          v-if="$store.state.auth.user.permissions && $store.state.auth.user.permissions.includes('loan-type_create')"
          slot="activator"
          color="primary"
          dark
          class="mb-2"
    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center align-center layout px-0">
          <v-icon
            v-if="$store.state.auth.user.permissions && $store.state.auth.user.permissions.includes('loan-type_update')"
            small
            class="mr-2"
          ...
          <v-icon
            v-if="$store.state.auth.user.permissions && $store.state.auth.user.permissions.includes('loan-type_delete')"
            small
            class="mr-2"
...
export default {
  ...
  async mounted() {
    await this.$store.dispatch('loans/getLoanTypes')
    this.loading = false
  },
  methods: {
    editItem(item) {
      this.editedIndex = this.loanTypes.indexOf(item)
      this.editedItem = { ...this.loanTypes[this.editedIndex] }
      this.dialog = true
    },
    deleteItem(item) {
      this.confirm = true
    ...
    async del(item) {
      ...
      await this.$store.dispatch('loans/deleteLoanType', item.id).catch(() => {
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        ...
        this.buttonLoading = true
        res = await this.$store
          .dispatch('loans/newLoanType', this.editedItem)
          .catch(() => {
            this.buttonLoading = false
          })
        this.buttonLoading = false

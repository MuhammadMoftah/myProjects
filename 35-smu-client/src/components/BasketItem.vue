<template>
  ...
  >
    ...
    <!-- Note  -->
    <div class="q-px-sm" v-if="item.note">
      <span class="roboto text-blue-grey-9"> Note: </span>
      <span class="text-grey-7">{{ item.note }}</span>
    </div>
    <div class="items-center justify-between row">
      ...
      <q-btn
        ...
        v-if="!note.length"
        no-caps
        @click="(noteDialog = true), (oldNote = note)"
      >
        <template slot="default">
          <q-icon name="description" size="xs" />
          <span class="roboto"> {{ $t("add_note") }} </span>
        </template>
      </q-btn>

      <q-btn
        class="text-green-6 q-mt-sm"
        flat
        v-else
        dense
        no-caps
      ...
      >
        <template slot="default">
          ...
          <span class="roboto"> {{ $t("edit_note") }} </span>
        </template>
      </q-btn>
      ...
      try {
        ...
        if (this.$store.state.general.scanType === "table") {
          const res = await this.$axios.patch(
            `/${this.$store.state.general.lang}/cart/` + this.item.cart_id,
            {
              id: this.item.id,
              quantity: quantity,
            }
        ...
        if (this.$store.state.general.scanType === "pickup") {
          const res = await this.$axios.patch(
            `/${this.$store.state.general.lang}/pickup/cart/` +
              this.item.cart_id,
            {
              id: this.item.id,
              quantity: quantity,
